; Test harness for SSH server-side key exchange
; Reads from stdin: socket_fd (4 bytes LE) + host_private_key (32 bytes) + host_public_key (32 bytes)
; Calls ssh_kex_server(fd, &state, &host_keypair)
...
_start:
    ; Read socket fd (4 bytes LE) + host_private_key (32 bytes) + host_public_key (32 bytes)
    ; Total: 68 bytes from stdin
    ...
    ; Extract socket fd
    mov edi, [rel input_buf]

    ; Copy host keypair (private 32 + public 32 = 64 bytes)
    lea rsi, [rel input_buf + 4]
    lea rdi, [rel host_keypair]
    mov ecx, 64
    rep movsb
