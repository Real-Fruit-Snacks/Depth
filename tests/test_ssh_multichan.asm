; Test harness for multi-channel SSH event loop (v2)
; Same architecture as test_ssh_e2e.asm:
;   Assembly side = program CLIENT (kex_client + auth_client + event_loop_v2)
;   Python side = teamserver SERVER (kex_server + auth_server + sends CHANNEL_OPEN)
;
; Reads from stdin:
;   sock_fd (4 bytes LE) + user_len(4 LE) + username + pass_len(4 LE) + password
;
; After kex_client + auth_client:
;   Initializes channel table
;   Enters ssh_client_event_loop_v2
;
